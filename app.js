(() => {
  ...
  function applyRoleUi(){
    ...
    const quick=document.querySelector('[data-action="quick-add"]');if(quick)quick.style.display=canEdit?'':'none';
  ...
  function renderDashboard(){
    ...
    const dealReady=deals.filter(({x})=>/TIẾP TỤC/.test(String(x.decision||'').toUpperCase()));
  ...
  function renderOperations(){
    ...
    const roadmapRows=projects.map(p=>`<tr><td><button class="linkish" data-open-project="${p.id}">${esc(p.id)}</button></td><td><b>${esc(p.name)}</b><div class="small muted">${esc(p.genre||'')} · ${esc(p.monetizationModel||'—')}</div></td><td>${operationRoadmapHtml(p,{mini:true})}</td><td><b>${esc(p.gatePhase||operationGateLabel(operationStageValue(p)))}</b><div class="small muted">${esc(p.deploymentStatus||'—')}</div></td><td>${badge(p.gateReview?.result||p.latestDecision||'Chưa review')}<div class="small muted">${esc(p.gateReview?.reason||'')}</div></td><td>${esc(clipText(p.nextAction||'—',110))}</td><td><button class="ghost" data-open-project="${p.id}">Review</button></td></tr>`);
