(() => {
  ...
  function marketGrowthModel(m,growthPools){
    const defs=[
      ['dl3','DL Growth 3M','DL_Growth_3m',25,'DL'],['dl6','DL Growth 6M','DL_Growth_6m',15,'DL'],['dl12','DL Growth 12M','DL_Growth_12m',10,'DL'],
      ['rev3','Revenue Growth 3M','Rev_Growth_3m',25,'REV'],['rev6','Revenue Growth 6M','Rev_Growth_6m',15,'REV'],['rev12','Revenue Growth 12M','Rev_Growth_12m',10,'REV']
    ];
    const components=defs.map(([key,label,field,weight,side])=>{const raw=marketGrowthValue(m,field);const score5=marketRelativeScore5(raw,growthPools[key]||[]);return {key,label,field,weight,side,raw,score5};});
  ...
  function marketMonetizationModel(m,rpdValues,grossingValues){
    ...
    const parts=[];if(rpdScore5!==null)parts.push({key:'rpd',label:'RPD',raw:rpd,score5:rpdScore5,weight:60});if(grossingScore5!==null)parts.push({key:'grossing',label:'Top100 Grossing Presence',raw:grossing,score5:grossingScore5,weight:40});
  ...
  function marketAnalytics(mechanics){
    ...
    return list.map(m=>{
      ...
      const out={m,dl,rev,dlg,revg,rpd,cpi,scale,growth,growth5,growthModel,monetization,monetization5,monetizationModel,grossing:monetizationModel.grossing,ua,uaModel,uaBenchmark:uaModel.profile,strategicFit,strategicFitInfo,executionFit,executionFitInfo,score,completeness,trend,confidence:marketConfidence(m)};out.direction=marketDirection(out);return out;
    });
  }
  ...
  function renderMarket(){
    ...
    const executiveNote=`<div class="market-exec-note"><b>Đọc nhanh cho quyết định:</b> ${top?`Cơ hội tổng hợp cao nhất hiện tại là <strong>${esc(top.m.Mechanic)}</strong> (${fmt(top.score,1)}/100).`: 'Chưa đủ dữ liệu để xếp hạng.'} ${topGrowth?`Đà tăng trưởng tương đối nổi bật: <strong>${esc(topGrowth.m.Mechanic)}</strong> (${fmt(topGrowth.growth,0)}/100 · ${esc(topGrowth.trend.label)}).`:''}<span><b>Growth /100</b> dùng 6 cửa sổ DL/Revenue 3M–6M–12M với trọng số ưu tiên dữ liệu gần nhất; thiếu metric thì tự chia lại trọng số nhưng phải có ít nhất 1 tín hiệu DL + 1 tín hiệu Revenue. <b>Khả năng kiếm tiền /100</b> = 60% RPD + 40% hiện diện Top100 Grossing, tự chia lại nếu thiếu một metric.</span></div>`;
    const formula=`<div class="market-formula"><span><b>Trọng số 30%</b> Quy mô</span><span><b>Trọng số 25%</b> Đà tăng trưởng</span><span><b>Trọng số 20%</b> Khả năng kiếm tiền</span><span><b>Trọng số 15%</b> UA</span><small><b>Sức hấp dẫn /100:</b> (Quy mô×30% + Growth×25% + Monetization×20% + UA×15%) / tổng trọng số có dữ liệu. <b>Growth:</b> DL 3M — trọng số 25% · DL 6M — 15% · DL 12M — 10% · Revenue 3M — 25% · Revenue 6M — 15% · Revenue 12M — 10%; từng signal được chấm 1–5 theo vị trí tương đối trong dataset rồi quy đổi /100. <b>Monetization:</b> RPD — trọng số 60% · Top100 Grossing Presence — 40%. <b>UA:</b> dùng benchmark CPI theo dòng game ở US: Puzzle $10–15 = Trung bình; Tycoon / Simulation / Strategy / RPG-TD $4–6 = Trung bình (~$5). CPI thấp hơn benchmark → điểm tăng; cao hơn → điểm giảm. Thiếu metric = bỏ khỏi mẫu số, không quy về 0.</small></div>`;
  ...
  function openMarketScoreDrilldown(id){
    ...
    const parts=[
      ...
      {label:'Đà tăng trưởng',score:x.growth,weight:25,detail:growthDetail},
      {label:'Khả năng kiếm tiền',score:x.monetization,weight:20,detail:monDetail},
      {label:'Hiệu quả UA',score:x.ua,weight:15,detail:`CPI median ${x.cpi===null?'—':'$'+fmt(x.cpi,2)}${x.uaBenchmark?` · ${x.uaBenchmark.label} bình thường $${fmt(x.uaBenchmark.normalLow,0)}–${fmt(x.uaBenchmark.normalHigh,0)}`:' · chưa map benchmark dòng game, fallback percentile'}`}
    ];
  ...
  function openGameMarketScoreDrilldown(id){
    ...
    const defs=[
      ['Quy mô thị trường',insight?.scale,30,'Downloads 30D + Revenue 30D · xếp hạng tương đối'],
      ['Đà tăng trưởng',insight?.growth,25,'Growth weighted-recency · DL + Revenue 3M/6M/12M'],
      ['Khả năng kiếm tiền',insight?.monetization,20,'Trọng số 60% RPD + 40% Top100 Grossing Presence'],
      ['Hiệu quả UA',insight?.ua,15,insight?.uaBenchmark?`CPI median · ${insight.uaBenchmark.label} bình thường $${fmt(insight.uaBenchmark.normalLow,0)}–${fmt(insight.uaBenchmark.normalHigh,0)}`:'CPI median · fallback percentile khi chưa map được dòng game']
    ];
  ...
  function marketEditGuideHtml(x){
    ...
    return `<div class="market-edit-guide">
      ...
      <details open><summary>Rule 2 · Khả năng kiếm tiền /100 được tính như thế nào?</summary><div class="market-rule-body"><p>Monetization dùng hai tín hiệu riêng: <b>RPD 60%</b> + <b>Top100 Grossing Presence 40%</b>. Mỗi tín hiệu được chấm 1–5 theo vị trí tương đối trong dataset rồi quy đổi /100. Nếu thiếu một metric, trọng số tự chia lại cho metric còn lại. <b>RPD không được cộng thêm vào Growth</b> để tránh double-count.</p><div class="market-rule-bands"><span><b>≥80</b> Rất tốt</span><span><b>60–79.9</b> Tốt</span><span><b>40–59.9</b> Trung bình</span><span><b>&lt;40</b> Thấp</span></div></div></details>
