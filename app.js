(() => {
  ...
  function openMarketScoreDrilldown(id){
    ...
    const growthRows=x.growthModel.components.map(c=>`<tr><td>${esc(c.label)}</td><td>${c.raw===null?'—':pct(c.raw,1)}</td><td>${c.score5===null?'—':fmt(c.score5,1)+'/5'}</td><td>${c.weight}%</td></tr>`).join('');
    const monRows=x.monetizationModel.parts.length?x.monetizationModel.parts.map(c=>`<tr><td>${esc(c.label)}</td><td>${c.key==='rpd'?'$'+fmt(c.raw,2):pct(c.raw,1)}</td><td>${fmt(c.score5,1)}/5</td><td>${c.weight}%</td></tr>`).join(''):'<tr><td colspan="4">Chưa đủ dữ liệu Monetization</td></tr>';
