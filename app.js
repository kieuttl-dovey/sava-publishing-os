(() => {
  ...
  // v0.49 · One source of truth for Market Attractiveness across Dashboard, Market, Game Selection, Sourcing, exports and snapshots.
  ...
  function marketUaScore(m,cpi,cpiVals){
    ...
    return {score:marketPctRank(cpi,cpiVals,{inverse:true}),profile:null,method:'dataset-percentile'};
  }
  ...
  function openMarketScoreDrilldown(id){
    ...
    const parts=[
      ...
      {label:'Hiệu quả UA',score:x.ua,weight:MARKET_SCORE_WEIGHTS.ua,detail:`CPI median ${x.cpi===null?'—':'$'+fmt(x.cpi,2)}${x.uaBenchmark?` · ${x.uaBenchmark.label} bình thường $${fmt(x.uaBenchmark.normalLow,0)}–${fmt(x.uaBenchmark.normalHigh,0)}`:' · chưa map benchmark dòng game, fallback percentile'}`}
    ];
  ...
  function openGameMarketScoreDrilldown(id){
    ...
    const defs=[
      ...
      ['Hiệu quả UA',insight?.ua,MARKET_SCORE_WEIGHTS.ua,insight?.uaBenchmark?`CPI median · ${insight.uaBenchmark.label} bình thường $${fmt(insight.uaBenchmark.normalLow,0)}–${fmt(insight.uaBenchmark.normalHigh,0)}`:'CPI median · fallback percentile khi chưa map được dòng game']
    ];
  ...
  function renderGames(){
    ...
    const rows=db.games.filter(g=>includesSearch(g.id,intake(g,'Game_Title'),intake(g,'Studio'),intake(g,'Mechanic'),gameDerived(g).selectionDecision,gameDerived(g).recommendation)).map(g=>{
      ...
      const marketScore=d.market===null?'—':`<button type="button" class="score-drill-trigger market quality-${marketBand.key}" data-open-game-market-score="${esc(g.id)}" title="Xem cấu thành Market Score">${identityScoreBar(d.market,'market')}<span>${esc(marketBand.label)} · Xem cấu thành ↗</span></button>`;
  ...
  function sourcingMarketAlignment(x){
    ...
    const marketItem=(db.market||[]).find(m=>mechanicNamesMatch(exactName,m.Mechanic));
    if(marketItem){
  ...
  function openGame(id){
    ...
    const body=`<div class="notice"><b>Hướng dẫn:</b> Lựa chọn trò chơi là bước lựa chọn Game <b>trước Test</b>. PRE-SCAN dùng Market + Publishing Readiness + Deal Economics + SAVA Fit. Các điểm 1–5 ưu tiên <b>AUTO SCORE từ dữ liệu nguồn</b>; chỉ Override thủ công khi có evidence tốt hơn và phải ghi lý do. Product Test để trống tới khi có data thực tế ở 08/09. Khi có ≥2/5 nhóm Product Evidence, workbook tự bật nhánh POST-TEST để bổ sung lớp evidence. <b>Hard Gate FAIL luôn override mọi score.</b></div>${derivedSummary}<div class="source-edit-note compact"><b>ℹ️ Pre-Scan, kết luận lựa chọn và Product Evidence summary là kết quả tự tính, không sửa trực tiếp.</b> Market Score chỉnh tại <b>Phân tích thị trường</b> và được đọc lại nguyên giá trị; Deal inputs chỉnh tại <b>Đàm phán & Thỏa thuận</b>; các field Candidate / Hard Gate / SAVA Fit / Product Evidence Override chỉnh ngay tại <b>Lựa chọn trò chơi</b>.</div>
      ...
      <div class="section-title">Thị trường /100 · Dữ liệu tham chiếu</div><div class="source-edit-note compact"><b>🔒 Market Score không sửa tại Lựa chọn trò chơi.</b> Đây là điểm dùng chung, được tính duy nhất tại <b>Phân tích thị trường</b> theo mechanic. Chỉnh Downloads/Revenue, Growth, RPD, Top100 Grossing hoặc CPI tại Phân tích thị trường → điểm Market của Game và Pre-Scan tự cập nhật. Các override Market cũ (nếu có) được giữ để truy vết nhưng <b>không còn tham gia tính điểm từ v0.44</b>.</div><div class="sourcing-ref-grid"><div><span>Market Score hiện tại</span><b>${gameDerived(g).market===null?'—':fmt(gameDerived(g).market,1)+'/100'}</b><small>Từ Phân tích thị trường</small></div><div><span>Mechanic liên kết</span><b>${esc(intake(g,'Mechanic')||'—')}</b></div><div><span>Evidence Market</span><b>${gameDerived(g).marketEvidenceCount}/4 cấu phần</b></div><div><span>Entry Accessibility</span><b>${gameAutoScore(g,'entryAccess')===null?'—':fmt(gameAutoScore(g,'entryAccess'),1)+'/5'}</b><small>Evidence phụ · không tính Market Score</small></div></div>
  ...
  function openSourcing(id){
    ...
    const body=`${sourcingEditGuide()}<div class="sourcing-derived"><div><span>Qualified</span><b>${esc(q)}</b></div><div><span>Giai đoạn hiện tại</span><b>${esc(stage)}</b></div><div><span>Cảnh báo Pipeline</span><b>${esc(alert)}</b></div><div><span>Phù hợp thị trường</span><b>${esc(align.label)}${align.score===null?'':` · ${align.score}/100`}</b></div></div>
      <div class="section-title">1. Thông tin Lead</div><div class="form-grid three-cols">${fText('id','Lead ID',x.id)}${fText('gameName','Game',x.game||x.leadName||'')}${fText('studio','Studio / Đối tác',x.studio||'')}${fText('country','Quốc gia',x.country||'')}${fText('genre','Genre',x.genre||'')}${fSelect('source','Nguồn',x.source||'',SOURCING_SOURCES)}${fText('owner','BD phụ trách',x.owner||'')}${fSelect('partner','Link Lựa chọn đối tác',x.partnerId||'', ['',...db.partners.map(p=>p.id)])}${fSelect('gameLink','Link Lựa chọn trò chơi',x.gameId||'', ['',...db.games.map(g=>g.id)])}</div>
      ...
      <div class="section-title">5. Tham chiếu Phân tích thị trường</div><div class="source-edit-note compact"><b>🔒 Đây là dữ liệu tham chiếu, không sửa tại Tìm kiếm cơ hội.</b> Benchmark, Growth, RPD, CPI và định hướng market chỉnh tại <b>Phân tích thị trường</b>.</div><div class="sourcing-ref-grid"><div><span>Market / nhóm chiến lược</span><b>${esc(align.group)}</b></div><div><span>Định hướng / mức phù hợp</span><b>${esc(align.label)}</b></div><div><span>Điểm tham chiếu</span><b>${align.score===null?'—':align.score+'/100'}</b><small>${align.source==='market'?'Từ Phân tích thị trường':'Fallback chiến lược'}</small></div><div><span>Kết hợp Screening</span><b>${esc(sourcingScreeningAction(x))}</b></div></div>${x.historicalEvaluation?`<div class="section-title">6. Tham chiếu đánh giá lịch sử</div><div class="sourcing-ref-grid"><div><span>Kết quả</span><b>${esc(x.historicalEvaluation.result||'—')}</b></div><div><span>Mức hoàn thiện</span><b>${esc(x.historicalEvaluation.completion||'—')}</b></div><div><span>Tiềm năng</span><b>${esc(x.historicalEvaluation.potential||'—')}</b></div><div><span>Monetization</span><b>${esc(x.historicalEvaluation.monetization||'—')}</b></div></div>`:''}`;
