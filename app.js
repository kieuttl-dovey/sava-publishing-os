(() => {
  ...
  function marketAnalytics(mechanics){
    ...
    return list.map(m=>{
      ...
      const out={m,dl,rev,dlg,revg,rpd,cpi,scale,growth,growth5,growthModel,monetization,monetization5,monetizationModel,grossing:monetizationModel.grossing,ua,strategicFit,strategicFitInfo,executionFit,executionFitInfo,score,completeness,trend,confidence:marketConfidence(m)};out.direction=marketDirection(out);return out;
    });
  }
  ...
  function openGameMarketScoreDrilldown(id){
    ...
    const defs=[
      ...
      ['Hiệu quả UA',insight?.ua,15,'CPI median · CPI thấp hơn được xếp hạng cao hơn']
    ];
