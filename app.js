(() => {
  ...
  function openGameMarketScoreDrilldown(id){
    ...
    const defs=[
      ['Market Size','marketSize',20,'Quy mô thị trường · LIVE từ Phân tích thị trường'],
      ['Growth / Momentum','growth',20,'Đà tăng trưởng · LIVE từ Phân tích thị trường'],
      ['Entry Accessibility','entryAccess',25,'Mức độ có thể thâm nhập'],
      ['Market Monetization','marketMonetization',20,'Khả năng kiếm tiền · LIVE từ Phân tích thị trường'],
      ['UA / Creative Scalability','marketUaScalability',15,'UA benchmark / khả năng scale creative · LIVE từ Phân tích thị trường']
    ];
    ...
    const raw=market?`<div class="drill-source-grid market-evidence-grid"><div><span>Downloads 30D</span><b>${fmt(market.Downloads_30d,0)}</b></div><div><span>Revenue 30D</span><b>${money(market.Revenue_30d_USD)}</b></div><div><span>DL Growth 3M</span><b>${marketGrowthValue(market,'DL_Growth_3m')===null?'—':pct(marketGrowthValue(market,'DL_Growth_3m'),1)}</b></div><div><span>DL Growth 6M</span><b>${marketGrowthValue(market,'DL_Growth_6m')===null?'—':pct(marketGrowthValue(market,'DL_Growth_6m'),1)}</b></div><div><span>Revenue Growth 3M</span><b>${marketGrowthValue(market,'Rev_Growth_3m')===null?'—':pct(marketGrowthValue(market,'Rev_Growth_3m'),1)}</b></div><div><span>Revenue Growth 6M</span><b>${marketGrowthValue(market,'Rev_Growth_6m')===null?'—':pct(marketGrowthValue(market,'Rev_Growth_6m'),1)}</b></div><div><span>RPD</span><b>${market.Revenue_per_Download_SAME_COHORT==null||Number(market.Revenue_per_Download_SAME_COHORT)<=0?'—':'$'+fmt(market.Revenue_per_Download_SAME_COHORT,2)}</b></div><div><span>Top100 Grossing</span><b>${marketGrossingPresence(market)===null?'—':pct(marketGrossingPresence(market),1)}</b></div><div><span>CPI median</span><b>${market.UA_Benchmark?.CPI_Median==null?'—':'$'+fmt(market.UA_Benchmark.CPI_Median,2)}</b></div></div>`:'<div class="drill-warning">Chưa map được record Phân tích thị trường theo mechanic hiện tại.</div>';
    const visualBand=gameScoreVisualBand(d.market,'market');
    modalRoot.innerHTML=`<div class="modal-backdrop drilldown-backdrop"><div class="modal wide drilldown-modal game-score-modal"><div class="modal-head"><div><span class="eyebrow-mini">LỰA CHỌN TRÒ CHƠI · MARKET SCORE BREAKDOWN</span><h2>${esc(intake(g,'Game_Title')||g.id)}</h2><p>Market Score là lớp đánh giá thị trường của candidate; tách biệt hoàn toàn với SAVA Fit.</p></div><button class="icon-btn" data-close>×</button></div><div class="modal-body"><div class="game-score-hero market quality-${visualBand.key}"><div><span>Sức hấp dẫn thị trường của Game</span><strong>${d.market===null?'—':fmt(d.market,1)}</strong><em>/100</em><small>${esc(visualBand.label)}</small></div><div class="game-score-hero-meta"><span>Evidence <b>${d.marketEvidenceCount}/5</b></span><span>Mechanic <b>${esc(intake(g,'Mechanic')||'—')}</b></span><span>Nguồn <b>Phân tích thị trường</b></span></div></div><div class="drill-grid-main"><section><div class="drill-section-head"><h3>5 cấu phần Market Score</h3><small>Cần tối thiểu 3/5 nhóm. Khi thiếu metric, trọng số được tự chia lại trên các nhóm có dữ liệu.</small></div><div class="drill-metric-list">${rows}</div><div class="score-formula-card"><b>Cách tính Market Score của Game</b><code>Market Score = Σ(Điểm thành phần / 5 × trọng số điều chỉnh)</code><p>Trọng số gốc: Size 20% · Growth 20% · Entry 25% · Monetization 20% · UA 15%. <b>Size, Growth, Market Monetization và UA AUTO</b> được tính lại trực tiếp từ <b>Phân tích thị trường</b> mỗi lần mở/refresh dữ liệu; <b>Entry Accessibility</b> giữ từ nguồn Competition. Growth dùng trọng số theo recency: DL 3M/6M/12M = 25%/15%/10% và Revenue 3M/6M/12M = 25%/15%/10%. Monetization dùng trọng số 60% RPD + 40% Top100 Grossing Presence. Override thủ công vẫn được ưu tiên nếu có evidence.</p></div></section><aside><div class="drill-source-card"><h3>Evidence thị trường</h3>${raw}${insight?`<div class="drill-evidence-note"><span>Định hướng Market</span><p><b>${esc(insight.direction.label)}</b> · Market Attractiveness ${insight.score===null?'—':fmt(insight.score,1)}/100</p></div>`:''}</div><div class="drill-source-card"><h3>Data lineage</h3><p><b>Market Score</b> cho candidate dùng 5 score 1–5. <b>Market Size, Growth, Market Monetization và UA</b> là dữ liệu market-owned và được đồng bộ live từ <b>Phân tích thị trường</b>; chỉ <b>Entry Accessibility</b> giữ từ Competition. Vì vậy khi Market data/công thức thay đổi, Market Score và Pre-Scan của Game tự tính lại. <b>SAVA Fit</b> là score riêng và chỉ được ghép ở Pre-Scan.</p></div></aside></div></div><div class="modal-foot"><button class="ghost" data-close>Đóng</button>${market?'<button class="ghost" data-market-record>Mở Phân tích thị trường</button>':''}<button class="ghost" data-game-preview>Hồ sơ Game</button><button class="primary" data-game-edit>Chỉnh sửa Score / Override</button></div></div></div>`;
    modalRoot.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modalRoot.innerHTML='');
  ...
  function renderSources(){
    ...
    const mapRows=[
      ...
      ['Lựa chọn trò chơi','Candidate intake · Product/Market score · SAVA Publishing Fit','Market dùng làm evidence năng lực; Tìm kiếm cơ hội/Deal/Operation link theo Game ID'],
      ['Phân tích thị trường','Market economics · Growth · RPD · CPI · taxonomy mechanic','Tìm kiếm cơ hội đọc Định hướng/Market evidence; Lựa chọn trò chơi đọc Market inputs'],
