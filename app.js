(() => {
  ...
  function openMarket(id){
    ...
    const body=`<div class="notice"><b>Hướng dẫn BD:</b> chỉ nhập/cập nhật benchmark thị trường và ghi chú. <b>Sức hấp dẫn, Xu hướng 3M, Khả năng kiếm tiền, Phù hợp chiến lược, Năng lực thực thi và Định hướng đều tự tính/tự lấy từ module sở hữu.</b></div>${marketEditGuideHtml(currentInsight)}<div class="section-title">Dữ liệu dùng chung từ Tìm kiếm cơ hội</div><div class="source-edit-note compact"><b>🔒 Chỉ đọc tại Phân tích thị trường.</b> Phù hợp chiến lược lấy từ <b>Tìm kiếm cơ hội · 07_Market_Intel_Ref</b>; muốn thay đổi phải cập nhật mapping chiến lược ở nguồn Tìm kiếm cơ hội, không sửa tại đây.</div>${mechanicStrategicFitHtml(strategicInfo)}<div class="section-title">Dữ liệu dùng chung từ Lựa chọn trò chơi</div><div class="source-edit-note compact"><b>🔒 Chỉ đọc tại Phân tích thị trường.</b> Năng lực thực thi lấy từ <b>Lựa chọn trò chơi</b>; muốn thay đổi cần cập nhật SAVA Publishing Fit của Game/Candidate tương ứng.</div>${mechanicExecutionFitHtml(executionInfo)}<div class="section-title">Dữ liệu thị trường</div><div class="form-grid three-cols">${fText('name','Mechanic',m.Mechanic)}${fText('geo','Geography',m.Geography)}${fText('platform','Platform',m.Platform)}${fText('dl30','Downloads 30d',m.Downloads_30d??'','','number')}${fText('dl90','Downloads 90d',m.Downloads_90d??'','','number')}${fText('dl12','Downloads 12m',m.Downloads_12m??'','','number')}${fText('rev30','Revenue 30d USD',m.Revenue_30d_USD??'','','number')}${fText('rev90','Revenue 90d USD',m.Revenue_90d_USD??'','','number')}${fText('rev12','Revenue 12m USD',m.Revenue_12m_USD??'','','number')}${fText('dlg3','Tăng trưởng DL 3M (decimal)',m.DL_Growth_3m??'','','number')}${fText('revg3','Tăng trưởng Revenue 3M (decimal)',m.Rev_Growth_3m??'','','number')}${fText('rpd','RPD (Revenue / Download)',m.Revenue_per_Download_SAME_COHORT??'','','number')}${fText('cpi','CPI median',m.UA_Benchmark?.CPI_Median??'','','number')}</div><div class="section-title">Ghi chú định hướng</div><div class="form-grid">${fArea('savaNote','Ghi chú / bối cảnh bổ sung',m.SAVA_Direction_Note||'')}</div>`;
