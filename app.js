(() => {
  ...
  // Màu score trong Lựa chọn trò chơi thể hiện chất lượng tín hiệu, không thể hiện loại score.
  // Ngưỡng bám theo logic Pre-Scan của workbook: >75 có tín hiệu tốt; 60–75 cần kiểm chứng; <60 cần xem xét.
  function gameScoreVisualBand(value){
    const n=num(value);
    if(n===null)return {key:'na',label:'Chưa có dữ liệu'};
    if(n>75)return {key:'good',label:'Tín hiệu tốt'};
    if(n>=60)return {key:'watch',label:'Cần kiểm chứng'};
    return {key:'weak',label:'Cần xem xét'};
  }
  function identityScoreBar(value,type='market'){
    const n=num(value),w=n===null?0:Math.max(0,Math.min(100,n)),band=gameScoreVisualBand(value);
  ...
  function openGameSavaFitDrilldown(id){
    ...
    const visualBand=gameScoreVisualBand(effective100);
  ...
  function openGameMarketScoreDrilldown(id){
    ...
    const visualBand=gameScoreVisualBand(d.market);
  ...
  function openGameQuickView(id){
    ...
    const marketBand=gameScoreVisualBand(d.market),savaBand=gameScoreVisualBand(sava);
  ...
  function renderGames(){
    ...
    const rows=db.games.filter(g=>includesSearch(g.id,intake(g,'Game_Title'),intake(g,'Studio'),intake(g,'Mechanic'),gameDerived(g).selectionDecision,gameDerived(g).recommendation)).map(g=>{
      ...
      const savaBand=gameScoreVisualBand(sava),marketBand=gameScoreVisualBand(d.market);
    ...
    const drillGuide=`<div class="game-drill-guide"><span><b>Thị trường</b> = mức hấp dẫn của market/candidate</span><span><b>Phù hợp SAVA</b> = khả năng SAVA phát hành tốt candidate</span><span class="game-score-color-legend"><i class="good"></i>Tín hiệu tốt &gt;75 <i class="watch"></i>Cần kiểm chứng 60–75 <i class="weak"></i>Cần xem xét &lt;60</span><span>Click <b>score</b> để xem cấu thành + evidence · Click <b>Tên/ID Game</b> để xem hồ sơ nhanh.</span></div>`;
